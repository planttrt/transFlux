...
ydata.PK <- xydata.PK[,c('TR','dEVI', 'dNDVI')]

modelList <- list(ng = 2000, burnin = 1000, typeNames = c('CA',rep('CON',2)))

out.BW <- gjamGibbs( ~ dT + Solar + Thermal + I(windy*dT) + I(rainy*dT) + I(cloud*dT),
                  xdata = xydata.BW, ydata = ydata.BW,
                  modelList = modelList)

out.PK <- gjamGibbs( ~ dT + Solar + Thermal + I(windy*dT) + I(rainy*dT) + I(cloud*dT),
...
# 
# transSumm <- as.data.table(t(apply(out$chains$bgibbs, quantile, MARGIN = 2, probs=c(.025,.975))))
