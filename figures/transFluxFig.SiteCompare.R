...
par(mar=c(4,4,3,1))
par(mfrow=c(1,2))
boxplot(both.betaStd[, c("BW-Solar", "PK-Solar")], outline=F, ylim=c(.2,.5), names=c('Piedmont', 'Coast'), col=colList[c(1,3)])
mtext(text = 'Solar Radiation', line = 1, font=2, cex=1.3)
boxplot(both.betaStd[, c("BW-LST4", "PK-LST4")], outline=F, ylim=c(.2,.5), names=c('Piedmont', 'Coast'), col=colList[c(1,3)])
mtext(text = 'Thermal Radiation', line = 1, font=2, cex=1.3)
